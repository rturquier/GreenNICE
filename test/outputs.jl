...
df = DataFrame(Country = Int64[], Decile = Int64[], EDE_init = Float64[], Pop = Float64[])
...
EDE_globaldecile = DataFrame(year = Int64[], Country = Int64[], Decile = Int64[], EDE = Float64[])

for t in 1:size(EDE_decilecountry,1)
    for c in 1:size(EDE_decilecountry, 2)
        for d in 1:size(EDE_decilecountry, 3)
            push!(EDE_globaldecile, (t, c, d, EDE_decilecountry[t, c, d]))
        end
    end
end
